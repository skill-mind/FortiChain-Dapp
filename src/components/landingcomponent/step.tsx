...
const StepProcess = () => {
  ...
  const [step1Ref, step1InView] = useInView({ threshold: 0.3, triggerOnce: true });
  const [step2Ref, step2InView] = useInView({ threshold: 0.3, triggerOnce: true });
  const [step3Ref, step3InView] = useInView({ threshold: 0.3, triggerOnce: true });
  const [step4Ref, step4InView] = useInView({ threshold: 0.3, triggerOnce: true });
  ...
  return (
    <section className="text-white py-16 px-4 overflow-hidden">
      ...
      >
        <motion.div 
          className="bg-gradient-to-b from-[#6b6464] to-[#343232] p-6 rounded-2xl shadow-lg md:flex md:justify-between md:items-center"
          ref={step1Ref}
          initial="hidden"
        ...
        >
          ...
          <motion.div 
            className="bg-[#FFFFFF33] p-4 rounded-lg mt-6 md:mt-0 md:w-1/3"
            initial={{ opacity: 0, x: 50 }}
            animate={step1InView ? { opacity: 1, x: 0 } : { opacity: 0, x: 50 }}
          ...
          >
            ...
            />
            <motion.button 
              className="w-full py-2 bg-[#0000FF] rounded-md font-medium hover:bg-blue-700 transition"
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
            >
        ...
        >
          ...
          <motion.div 
            className="bg-gray-800 p-4 rounded-lg mt-6 md:mt-0 md:w-1/3 bg-[#FFFFFF33]"
            initial={{ opacity: 0, x: 50 }}
            animate={step2InView ? { opacity: 1, x: 0 } : { opacity: 0, x: 50 }}
            transition={{ delay: 0.4, duration: 0.6 }}
          >
        ...
        >
          ...
          <motion.div 
            ...
            animate={step3InView ? { opacity: 1, scale: 1 } : { opacity: 0, scale: 0.5 }}
        ...
        >
          ...
          <motion.div 
            className="mt-6 md:mt-0"
            initial={{ opacity: 0, scale: 0.5 }}
            animate={step4InView ? { opacity: 1, scale: 1 } : { opacity: 0, scale: 0.5 }}
            transition={{ delay: 0.3, duration: 0.6 }}
          >
            <motion.div
              animate={{ 
