...
const Details: React.FC<Props> = ({ dataIndex, setCurrentView }) => {
	...
	const report: ReportData | undefined = dataIndex !== null ? reports[dataIndex] : undefined;
	...
	if (!report) {
		return <div className="p-4 bg-gray-900 text-white min-h-screen">No report selected.</div>;
	}

	return (
		<motion.main variants={mainContainerVariants} initial="hidden" animate="visible">
			<motion.button
				...
				>
					...
					>
						...
						>
							...
							<span className="text-white font-semibold">Broken Access Control</span>
						</motion.div>
					</motion.div>
				</motion.div>
			...
			<Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
				...
				>
					<div className="flex items-center">
						...
						<h2 className="text-xl font-bold md:text-2xl lg:text-[32px]">DeFi Guard</h2>
					</div>
					<button
				...
				>
					A decentralized finance (DeFi) protection tool that scans for vulnerabilities in
					DeFi protocols and helps prevent hacks.
				</motion.p>
				<motion.div
					variants={modalItemVariants}
